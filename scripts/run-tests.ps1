...
if (-not (Test-Path -LiteralPath $enginePath -PathType Leaf)) {
    throw "Missing Engine\ffmpeg.exe. Copy a Windows x64 FFmpeg build with libsoxr support there first."
}

$versionOutput = & $enginePath -version 2>&1 | Out-String
if ($LASTEXITCODE -ne 0 -or $versionOutput -notmatch '--enable-libsoxr') {
    throw 'Engine\ffmpeg.exe is unavailable or was built without libsoxr support.'
}
